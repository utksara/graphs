...
int calculate_min_gates(int N_total, int BT, int RT, int RW) {
	...
	if (N_total > 20)
		int init_value = calculate_min_gates(N_total / 2, BT, RT, RW);

	for (int N_air = 1; N_air < N_total; N_air++) {
		aircraft::freedplane = 0;
	//	std::cout << "\For : " << N_air << " in air ";
		...
		for (int N_gates = init_value; N_gates <= N_total; N_gates++) {
		...
		//	std::cout << "\n Starting run for " << N_gates << " no. of gates :";
			...
			while (aircraft::freedplane != N_total) {
			...
			/*	if(q1->accumulation()>0)
				std::cout << " \n For queue 1  accumulation is: " << q1->accumulation();
				if (q2->accumulation()>0)
				std::cout << " \n For queue 2  accumulation is: " << q2->accumulation(); */
	...
	int optimal_gates = *(std::max_element(Gates_array, Gates_array + N_total - 1));

		std::cout << " for "<<N_total<<" number of planes, "<<"optimal gates is "<< optimal_gates<<"\n..............\n";
